<script>
  ...
  function showSize (rawSize) {
    if (rawSize < 1000) {
      ...
      return `${(rawSize / 1024).toFixed(2)} mb`
    }
  }
</script>
...
{:else}
  <div class="flex flex-col items-stretch max-w-4xl my-0 mx-auto">
    ...
    <div class="doc flex flex-wrap p-2" on:click={selectDoc(resource)} class:selected="{current === resource.nombre}">
      ...
      <span class="truncate cursor-pointer ml-4 hover:underline">{resource.nombre}</span>
      ...
      {#if current === resource.nombre}
        <div class="details p-4 flex items-center w-full relative">
          ...
          <div class="file-data mx-4">
            ...
            <span class="block mb-4">{dayjs(resource.updatedAt).fromNow()}</span>
            <span class="font-bold">Formato:</span>
            <span class="mr-4">{resource.archivo.ext.replace('.', '')}</span>
